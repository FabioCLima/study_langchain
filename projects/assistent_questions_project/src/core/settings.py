...
class ProjectSettings(BaseSettings):
    ...
    langsmith_project: str = Field(
        default="Assistant Questions Project", description="LangSmith project name"
    )

    # LLM Configuration
    llm_model: str = Field(default=DEFAULT_MODEL, description="OpenAI model to use")
    ...
    llm_max_tokens: int | None = Field(
        default=None, gt=0, description="Maximum tokens in LLM response"
    )

    # Logging Configuration
    log_level: str = Field(default=DEFAULT_LOG_LEVEL, description="Logging level")
    log_file: str = Field(
        default="assistant_questions.log", description="Log file path"
    )
    ...
    def get_langsmith_key(self) -> str | None:
    ...
    def mask_sensitive_data(self) -> dict[str, str]:
        ...
        """
        masked_openai = (
            f"***{self.get_openai_key()[-4:]}" if self.openai_api_key else "Not set"
        )
