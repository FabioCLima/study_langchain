...
prompt_city = ChatPromptTemplate.from_messages([
    ...
    ("user", "Responda APENAS em JSON com a chave 'curiosidade'. "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Obter capital e curiosidade de um país")
    parser.add_argument("--pais", type=str, required=True, help="Nome do país")
    args = parser.parse_args()
    
    print("✅ Configurações carregadas:")
    print("OPENAI_API_KEY:", settings.openai_api_key.get_secret_value()[:4] + "..." + settings.openai_api_key.get_secret_value()[-4:])
    print("LANGSMITH_API_KEY:", settings.langsmith_api_key.get_secret_value()[:4] + "..." + settings.langsmith_api_key.get_secret_value()[-4:])
