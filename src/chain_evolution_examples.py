...
def example_3_multi_step_chain():
    ...
    chain = (
        ...
        | model
        | StrOutputParser()
        | {"analysis": RunnablePassthrough(), "examples": RunnablePassthrough(), "concept": RunnablePassthrough()}
        | synthesis_prompt
        | model
        | StrOutputParser()
    )
...
def safe_chain_execution(chain, input_data: Dict[str, Any], max_retries: int = 3) -> str:
